...
from .utils import cache, helpers, commands
...
async def plonk_iterator(bot: RoboHashira, guild: discord.Guild, records: list[Record]) -> AsyncIterator[str]:
...
class ModifyType(enum.Enum):
    """The type of modification to make to the temp channel list."""
    ADD = 1
    REMOVE = 2
    EDIT = 3


class TempChannel(NamedTuple):
    id: int
    format: str
...
class GuildConfig:
    def __init__(self, cog: Config, record: asyncpg.Record):
        self.cog: Config = cog
    ...
    async def edit(
            self,
            music_channel: Optional[int] = MISSING,
            music_message_id: Optional[int] = MISSING,
            temp_channels: Optional[List[tuple[TempChannel, ModifyType]]] = MISSING
    ) -> Self:
        if music_channel is not MISSING:
            self.music_channel = music_channel

        if music_message_id is not MISSING:
            self.music_message_id = music_message_id

        if temp_channels is not MISSING:
            for channel in temp_channels:
                if channel[1] == ModifyType.ADD:
                    self.temp_channels.append(channel[0])
                elif channel[1] == ModifyType.REMOVE:
                    self.temp_channels.remove(channel[0])
                elif channel[1] == ModifyType.EDIT:
                    self.temp_channels[self.temp_channels.index(channel[0])] = channel[0]

        query = """
            INSERT INTO guild_mod_config (id, music_channel, music_message_id, temp_channels)
            VALUES ($1, $2, $3, $4)
            ON CONFLICT (id)
                DO UPDATE SET 
                    music_channel=$2, 
        ...
        await self.bot.pool.execute(query, self.id, self.music_channel, self.music_message_id, self.temp_channels)
        self.cog.get_config.invalidate(self.cog, self.id)
...
class Config(commands.Cog):
    ...
    async def on_guild_channel_delete(self, channel: discord.abc.GuildChannel):
        ...
        config: GuildConfig = await self.get_config(channel.guild.id)
        m_channel = discord.utils.get(config.temp_channels, id=channel.id)
        if m_channel:
            return config.temp_channels.remove(m_channel)
    ...
    @commands.command(ignore.command, name='list')
    ...
    async def ignore_list(self, ctx: GuildContext):
        ...
        class PlonkedPaginator(BasePaginator):

            async def format_page(self, entries: List, /) -> discord.Embed:
                entries = plonk_iterator(ctx.bot, ctx.guild, entries)
                embed = discord.Embed(timestamp=discord.utils.utcnow(), colour=helpers.Colour.teal())
    ...
    @commands.command(ignore.command, name='all')
    ...
    @commands.command(ignore.command, name='clear')
    ...
    @commands.command(config.group, pass_context=True, invoke_without_command=True, aliases=['unplonk'])
    ...
    @commands.command(unignore.command, name='all')
    ...
    @commands.command(channel.command, name='disable')
    ...
    @commands.command(channel.command, name='enable')
    ...
    @commands.command(server.command, name='disable')
    ...
    @commands.command(server.command, name='enable')
    ...
    @commands.command(config.command, name='enable')
    ...
    @commands.command(config.command, name='disable')
    ...
    @commands.command(config.command, name='disabled')
    ...
    @commands.command(_global.command, name='block')
    ...
    @commands.command(_global.command, name='unblock')
