...
from .utils import checks, converters, formats
...
class Music(commands.Cog):
    ...
    async def on_wavelink_track_end(self, payload: wavelink.TrackEndEventPayload):
        ...
        if player.queue.listen_together.enabled:
            ...
            try:
                ...
                return await player.view.channel.send("I couldn't find the track you were listening to on Spotify.")
        ...
        if before.id == listen_together.user_id:
            ...
            if new_activity and new_activity.title == before_activity.title:
                ...
                try:
                    ...
                    return await player.view.channel.send(
    ...
    async def play(self, ctx: Context, *, flags: PlayFlags):
        ...
        **You can play from sources such as YouTube, Spotify, SoundCloud, and more.**
        ...
        if not flags.query:
            return await ctx.send_tick(False, "Please provide a search query.", ephemeral=True,
                                       delete_after=10)
    ...
    async def connect(self, ctx: Context, channel: Union[discord.VoiceChannel, discord.StageChannel] = None):
        ...
        if ctx.voice_client:
            return await ctx.send(
                '<:redTick:1079249771975413910> I am already connected to a voice channel. Please disconnect me first.')

        try:
            channel = channel or ctx.author.voice.channel
        except AttributeError:
            return await ctx.send(
                '<:redTick:1079249771975413910> No voice channel to connect to. Please either provide one or join one.')

        await self.join(ctx)
        await ctx.send_tick(True, "Connected and bound to {channel.mention}", delete_after=10)
    ...
    async def leave(self, ctx: Context):
        ...
        await player.view.update(PlayingState.STOPPED)
        await player.disconnect()
        await ctx.send_tick(True, "Disconnected Channel and cleaned up the queue.",
                            delete_after=10)
    ...
    async def stop(self, ctx: Context):
        ...
        await player.disconnect()
        await ctx.send_tick(True, "Stopped Track and cleaned up queue.", delete_after=10)
    ...
    async def pause_or_resume(self, ctx: Context):
        ...
        await ctx.send(
            f"<:greenTick:1079249732364406854> {'Paused' if player.paused else 'Resumed'} Track "
    ...
    async def loop(self, ctx: Context, mode: Literal["Normal", "Track", "Queue"]):
        ...
        await player.view.update()
        await ctx.send_tick(True, "Loop Mode changed to `{mode}`", delete_after=10)
    ...
    async def shuffle(self, ctx: Context, mode: Literal["True", "False"]):
        ...
        await player.view.update()
        await ctx.send_tick(True, "Shuffle Mode changed to `{mode}`", delete_after=10)
    ...
    async def seek(self, ctx: Context, position: Optional[str] = None):
        ...
        if player.current.is_stream:
            return await ctx.send_tick(False, "Cannot seek if track is a stream.", ephemeral=True,
                                       delete_after=10)
        ...
        if position is None:
            ...
            try:
                ...
                return await ctx.send_tick(False, "Please provide a valid TimeStamp format.",
                                           ephemeral=True)
            ...
            if seconds in range(int(player.current.length)):
                ...
                return await ctx.send(
                    "<:redTick:1079249771975413910> Please provide a seek time within the range of the track.",
                    ephemeral=True, delete_after=10)

        await ctx.send(
            f"<:greenTick:1079249732364406854> Seeked to position ``{converters.convert_duration(seconds)}``",  # noqa
            delete_after=10)
        ...
        if not valid_volume(amount):
            return await ctx.send(
                embed=discord.Embed(
                    title="Invalid Input",
                    description=(
                        f"<:redTick:1079249771975413910> The Input `{amount}` is not a valid Volume Input.\n"
    ...
    async def leftcleanup(self, ctx: Context):
        ...
        await player.view.update()
        await ctx.send_tick(True, "Cleaned up the queue.", delete_after=10)
    ...
    async def filter_equalizer(
        ...
        if not band or not gain:
            return await ctx.send_tick(False, "Please provide a valid Band and Gain or a Preset.")
    ...
    async def filter_reset(self, ctx: Context):
        ...
        await ctx.send("<:greenTick:1079249732364406854> Removed all active filters.", delete_after=10)
    ...
    async def forceskip(self, ctx: Context):
        ...
        if not player.queue.future_is_empty:
            await ctx.send_tick(True, "An admin or DJ has to the next track.", delete_after=10)
            await player.skip()
        else:
            return await ctx.send_tick(False, "The queue is empty.", ephemeral=True, delete_after=10)
    ...
    async def jump_to(self, ctx: Context, position: int):
        ...
        if not player.queue.all_is_empty:
            if position < 0:
                return await ctx.send_tick(False, "The index must be greater than or 0.",
                                           ephemeral=True, delete_after=10)

            if (position - 1) > len(player.queue.all):
                return await ctx.send_tick(False, "There are not that many tracks in the queue.",
                                           ephemeral=True, delete_after=10)
            ...
            if position != 1:
                text = f"<:greenTick:1079249732364406854> Playing the **{position}** track in queue."
            else:
                text = "<:greenTick:1079249732364406854> Playing the next track in queue."
    ...
    async def back(self, ctx: Context):
        ...
        if not player.queue.history_is_empty:
            await ctx.send("<:greenTick:1079249732364406854> An admin or DJ has skipped to the previous song.",
                           delete_after=10)
            ...
            return await ctx.send_tick(False, "No items currently in the history of the queue.\n"
    ...
    async def queue(self, ctx: Context):
        ...
        if player.queue.all_is_empty:
            return await ctx.send_tick(False, "No items currently in the queue.", ephemeral=True)
    ...
    async def lyrics(self, ctx: Context, *, song: str = None):
        ...
        if not player:
            if not song:
                return await ctx.send_tick(False, "Please provide a song to search for.", ephemeral=True,
                                           delete_after=10)
        ...
        async with self.bot.session.get(
            ...
            if resp.status != 200:
                await mess.delete()
                return await ctx.send_tick(False, "I cannot find lyrics for the current track.",
                                           ephemeral=True, delete_after=10)
