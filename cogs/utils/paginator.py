...
class TextSource:
    ...
    def close_page(self) -> None:
        ...
        if self.prefix is not None:
            ...
            self._count = self.prefix_len + len(self.seperator)
        else:
            self._current_page = []
            self._count = 0
...
class BasePaginator(discord.ui.View, Generic[T]):
    ...
    def numerate_start(self) -> int:
        ...
        return self._current_page * self.per_page
...
class EmbedPaginator(BasePaginator[discord.Embed]):
    ...
    async def format_page(self, entries: List[discord.Embed], /) -> discord.Embed:
        return entries[0]
...
class FilePaginator(BasePaginator[AnyStr]):
    ...
    async def start(
        ...
        page = await self.format_page(self.pages[0])
        kwargs = {'files': page, 'view': self}  # type: ignore # lying
        if self.total_pages <= 1:
            kwargs.pop('view')
